...
class GAT_Model(nn.Module):
    ...
    def __init__(
        ...
        self.graph = graph
        # tqdm flag
        self.tqdm_disabled = kwargs.get('disable_tqdm',False)

        # Type of learning model
        self.model_type = 'graph_attention_network'
        ...
        for pname in DEFAULT_HYPERPARAMS.keys():
            if self.config[self.model_type]['hyperparameters'].get("gat_"+pname,None) is None:
                self.hyperparams[pname] = DEFAULT_HYPERPARAMS[pname]
                # Update config
                self.config[self.model_type]['hyperparameters'][pname] = DEFAULT_HYPERPARAMS[pname]
            else:
                self.hyperparams[pname] = self.config[self.model_type]['hyperparameters']["gat_"+pname]
        # create modules
    ...
    def get_loss(self, trip_od, trip_volume, in_flows, out_flows):
        '''
        ...
        edge_predict_loss = self.MSE(edge_prediction, trip_volume)
    ...
    def negative_sampling(self, train_y):
        ...
        if self.config[self.model_type].get('gat_negative_sampling_rate',0) == 0:
            return None
    ...
    def run_single(self, train_y, train_inflow, train_outflow):
        # turn model state
        self.train()
