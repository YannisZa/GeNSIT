...
from ticodm.math_utils import logsumexp
from ticodm.global_variables import NUMBA_PARALLELISE
...
def log_flow_matrix(xx,theta,origin_demand,cost_matrix,total_flow):
    ...
    log_utility = alpha*xx - beta*cost_matrix
    # Compute log normalisation factor
    normalisation = logsumexp(log_utility.ravel())
    # Evaluate log flow scaled
    log_flow = log_utility - normalisation + log_total + np.log(total_flow)
    return log_flow
