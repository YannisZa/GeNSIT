...
class Test5Helpers:
    ...
    def pytorch_log_poisson_pmf_total_constrained_sim(xx_tor,theta_tor,costmat_tor,table_tor):
        intensity_tor = Test4Helpers.pytorch_total_constrained_intensity(xx_tor,theta_tor,costmat_tor,torch.sum(table_tor))
    ...
    def pytorch_log_multinomial_pmf_total_constrained_sim(xx_tor,theta_tor,costmat_tor,table_tor):
        intensity_tor = Test4Helpers.pytorch_total_constrained_intensity(xx_tor,theta_tor,costmat_tor,torch.sum(table_tor))
        return Test5Helpers.pytorch_log_multinomial_unnormalised_pmf(intensity_tor,table_tor)
    
    def log_multinomial_pmf_total_constrained_sim_jacobian(total_constrained_sim,xx,theta,table):
        log_intensity = total_constrained_sim.log_intensity(xx,theta,np.sum(table))
        ...
        intensity_grad = Test4Helpers.total_constrained_intensity_gradient(total_constrained_sim,xx,theta,table)
    ...
    def log_multinomial_pmf_total_constrained_sim_jacobian2(total_constrained_sim,xx,theta,table):
        log_intensity = total_constrained_sim.log_intensity(xx,theta,np.sum(table))
        nrows,ncols = np.shape(log_intensity)
    ...
    def pytorch_log_product_multinomial_pmf_total_constrained_sim(xx_tor,theta_tor,costmat_tor,table_tor):
        intensity_tor = Test4Helpers.pytorch_total_constrained_intensity(xx_tor,theta_tor,costmat_tor,torch.sum(table_tor))
    ...
    def pytorch_log_fishers_hypergeometric_pmf_total_constrained_sim(xx_tor,theta_tor,costmat_tor,table_tor):
        intensity_tor = Test4Helpers.pytorch_total_constrained_intensity(xx_tor,theta_tor,costmat_tor,torch.sum(table_tor))
...
@pytest.fixture
def total_constrained_sim(sim_default_config):
    # Build a contingency table
    sim_default_config['table_distribution_name'] = "multinomial"
    sim = TotalConstrainedSIM(sim_default_config)
    return sim

@pytest.fixture
def total_constrained_log_intensity(total_constrained_sim):
    theta = np.array([0.8,0.5,0.1,10000,1.1,1.0])
    return total_constrained_sim.log_intensity(total_constrained_sim.log_destination_attraction,theta,100)
...
def test_odds_ratio(total_constrained_log_intensity,test5_helpers):

    # Get dimensions
    nrows,ncols = np.shape(total_constrained_log_intensity)

    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True)

    # My gradient
    my_log_or = log_odds_ratio_wrt_intensity(total_constrained_log_intensity)
...
def test_poisson_pmf(ct_default,total_constrained_log_intensity,test5_helpers):
    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True)
    ...
    my_log_pmf = log_poisson_pmf_unnormalised(total_constrained_log_intensity.astype('float32'),ct_default.table.astype('int32'))
...
def test_poisson_pmf_jacobian(ct_default,total_constrained_log_intensity,test5_helpers):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True).float()
    ...
    my_log_pmf_jacobian = log_poisson_pmf_jacobian_wrt_intensity(total_constrained_log_intensity,ct_default.table)
...
def test_poisson_pmf_jacobian_wrt_xx(test5_helpers,total_constrained_sim,ct_default):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    theta = np.array([0.8,0.5])
    xx = total_constrained_sim.log_destination_attraction
    xx_tor = torch.tensor(xx, requires_grad=True).float()
    theta_tor = torch.tensor(theta, requires_grad=False).float()
    costmat_tor = torch.tensor(total_constrained_sim.cost_matrix,requires_grad=False).float()
    table_tor = torch.tensor(ct_default.table,requires_grad=False).float()

    # Compute the Jacobian of z with respect to x
    true_jacobian_tor,_,_,_ = torch.autograd.functional.jacobian(
                                    test5_helpers.pytorch_log_poisson_pmf_total_constrained_sim,
                                    (xx_tor,theta_tor,costmat_tor,table_tor)
                            )
    true_jacobian = true_jacobian_tor.detach().cpu().numpy()

    # Compute my gradient
    log_intensity = total_constrained_sim.log_flow_matrix(
        total_constrained_sim.log_destination_attraction,
        theta,
        total_constrained_sim.origin_demand,
        total_constrained_sim.cost_matrix,
        np.sum(ct_default.table)
    )
    intensity_gradient_wrt_x = total_constrained_sim.flow_matrix_jacobian(theta,log_intensity)
...
def test_multinomial_pmf(ct_default,total_constrained_log_intensity,test5_helpers):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    ...
    # Normalised pmf
    # Compute log pmf using my method
    my_log_pmf = log_multinomial_pmf_unnormalised(total_constrained_log_intensity.astype('float32'),ct_default.table.astype('int32'))
...
def test_multinomial_pmf_jacobian(ct_default,total_constrained_log_intensity,test5_helpers):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    ...
    # Compute likelihood derivative wrt to log intensity using my method
    my_log_pmf_jacobian = log_multinomial_pmf_jacobian_wrt_intensity(total_constrained_log_intensity,ct_default.table)
...
def test_multinomial_pmf_jacobian_wrt_xx(test5_helpers,total_constrained_sim,ct_default):
    # Dimensions
    ...
    # Compute the Jacobian of z with respect to x
    true_jacobian_tor,_,_,_ = torch.autograd.functional.jacobian(
                                    test5_helpers.pytorch_log_multinomial_pmf_total_constrained_sim,
                                    (xx_tor,theta_tor,costmat_tor,table_tor)
                            )
    true_jacobian = true_jacobian_tor.detach().cpu().numpy()

    # Compute jacobian manually by deriving maths
    manual_jacobian = test5_helpers.log_multinomial_pmf_total_constrained_sim_jacobian(total_constrained_sim,xx,theta,ct_default.table)
    manual_jacobian2 = test5_helpers.log_multinomial_pmf_total_constrained_sim_jacobian2(total_constrained_sim,xx,theta,ct_default.table)
...
def test_product_multinomial_pmf(ct_default,total_constrained_log_intensity,test5_helpers):
    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True)
    table_tor = torch.tensor(ct_default.table,requires_grad=False)
    ...
    # Normalised pmf
    # Compute log pmf using my method
    my_log_pmf = log_product_multinomial_pmf_unnormalised(total_constrained_log_intensity.astype('float32'),ct_default.table.astype('int32'))
...
def test_product_multinomial_pmf_jacobian(ct_default,total_constrained_log_intensity,test5_helpers):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True).float()
    ...
    # Compute likelihood derivative wrt to log intensity using my method
    my_log_pmf_jacobian = log_product_multinomial_pmf_jacobian_wrt_intensity(total_constrained_log_intensity,ct_default.table)
...
def test_product_multinomial_pmf_jacobian_wrt_xx(test5_helpers,total_constrained_sim,ct_default):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    theta = np.array([0.8,0.5])
    xx = total_constrained_sim.log_destination_attraction
    ...
    # Compute the Jacobian of z with respect to x
    true_jacobian_tor,_,_,_ = torch.autograd.functional.jacobian(
                                    test5_helpers.pytorch_log_product_multinomial_pmf_total_constrained_sim,
                                    (xx_tor,theta_tor,costmat_tor,table_tor)
                            )
    true_jacobian = true_jacobian_tor.detach().cpu().numpy()
...
def test_fishers_hypergeometric_pmf(ct_default,total_constrained_log_intensity,test5_helpers):
    # Convert object to torch tensors
    intensity_tor = torch.tensor(np.exp(total_constrained_log_intensity),requires_grad=True)
    table_tor = torch.tensor(ct_default.table,requires_grad=False)
    ...
    # Normalised pmf
    # Compute log pmf using my method
    my_log_pmf = log_fishers_hypergeometric_pmf_unnormalised(total_constrained_log_intensity.astype('float32'),ct_default.table.astype('int32'))
...
def test_fishers_hypergeometric_pmf_jacobian_wrt_intensity(ct_default,total_constrained_log_intensity,test5_helpers):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    # Intensity function
    intensity = np.exp(total_constrained_log_intensity)
    # Convert object to torch tensors
    intensity_tor = torch.tensor(intensity,requires_grad=True).float()
    table_tor = torch.tensor(ct_default.table,requires_grad=False).float()

    # Compute likelihood derivative wrt to log intensity using my method
    my_log_pmf_jacobian = log_fishers_hypergeometric_pmf_jacobian_wrt_intensity(total_constrained_log_intensity,ct_default.table)
...
def test_fishers_hypergeometric_pmf_jacobian_wrt_xx(test5_helpers,total_constrained_sim,ct_default):
    # Dimensions
    nrows,ncols = np.shape(ct_default.table)
    theta = np.array([0.8,0.5])
    xx = total_constrained_sim.log_destination_attraction
    ...
    # Compute the Jacobian of z with respect to x
    true_jacobian_tor,_,_,_ = torch.autograd.functional.jacobian(
                                    test5_helpers.pytorch_log_fishers_hypergeometric_pmf_total_constrained_sim,
                                    (xx_tor,theta_tor,costmat_tor,table_tor)
                            )
    true_jacobian = true_jacobian_tor.detach().cpu().numpy()
