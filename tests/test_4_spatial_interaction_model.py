...
class Test4Helpers:
    ...
    @staticmethod
    def pytorch_total_constrained_intensity(xx_tor,theta_tor,cost_matrix_tor,table_total_tor):
        # Compute log pmf
    ...
    @staticmethod
    def pytorch_total_constrained_sde_potential(theta_tor,xx_tor,origin_demand_tor,cost_mat_tor):
        # Get parameters
        alpha = theta_tor[0]
        beta = theta_tor[1]
        delta = theta_tor[2]
    ...
    @staticmethod
    def total_constrained_intensity_gradient(total_constrained_sim,xx,theta,table):
        log_intensity = total_constrained_sim.log_intensity(xx,theta,table.sum())
...
def total_constrained_sim(sim_default_config):
    ...
    sim_default_config['sim_type'] = "TotalConstrained"
    sim = instantiate_sim(sim_default_config)
...
def test_total_constrained_log_expected_flows(total_constrained_sim):
    # Create log destination attraction
    # Create theta
    print(total_constrained_sim.cost_matrix)
    theta = np.array([0.8,0.5,0.1,10000,1.1,1.0])
    log_lambdas = total_constrained_sim.log_intensity(total_constrained_sim.log_destination_attraction,theta,100)
    ...
    print(total_constrained_sim.dims)
    nrows,ncols = total_constrained_sim.dims
    for i in range(nrows):
        for j in range(ncols):
            assert abs(log_lambdas[i,j]-true_log_lambdas[i,j]) <= 1e-5

def test_total_constrained_log_intensity_jacobian(test4_helpers,total_constrained_sim,table_2x3_n100):
    # Dimensions
    nrows,ncols = np.shape(total_constrained_sim.cost_matrix)
    ...
    costmat_tor = torch.tensor(total_constrained_sim.cost_matrix,requires_grad=False)
    table_total_tor = torch.tensor(total,requires_grad=False)
    
    torch_grad,_,_,_ = torch.autograd.functional.jacobian(test4_helpers.pytorch_total_constrained_intensity, 
                                                    (xx_tor,theta_tor,costmat_tor,table_total_tor))
    torch_intensity_grad = torch_grad.detach().cpu().numpy()

    # Manually computed gradient
    manual_intensity_grad = test4_helpers.total_constrained_intensity_gradient(total_constrained_sim,xx,theta,table_2x3_n100)

    # My gradient
    log_intensity = total_constrained_sim.log_intensity(
        total_constrained_sim.log_destination_attraction,
        theta,
        total
    )
    my_intensity_grad = total_constrained_sim.intensity_gradient(theta,log_intensity)
...
def test_total_constrained_potential_value(test4_helpers,total_constrained_sim):
    # Create log destination attraction
    # xx = np.array([-0.91629073,-0.51082562])
    # Create theta
    theta = np.array([1.0,0.5*100,0.1,10000,1.1,1.0])
    theta_tor = torch.tensor(theta,requires_grad=False)
    cost_mat_tor = torch.tensor(total_constrained_sim.cost_matrix,requires_grad=False)
    origin_demand_tor = torch.tensor(total_constrained_sim.origin_demand,requires_grad=False)
    xx_tor = torch.tensor(total_constrained_sim.log_destination_attraction,requires_grad=True)

    # Compute gamma*V_{theta}(x)
    pot,gradPot = total_constrained_sim.sde_potential_and_gradient(total_constrained_sim.log_destination_attraction,theta)
    pot_torch = test4_helpers.pytorch_total_constrained_sde_potential(theta_tor,xx_tor,origin_demand_tor,cost_mat_tor)
    true_pot = 52039.74450161611

    assert abs(pot-true_pot) <= 1e-1
    assert abs(pot_torch-true_pot) <= 1e-1

def test_total_constrained_potential_gradient(test4_helpers,total_constrained_sim):
    # Create theta
    theta = np.array([1.0,0.5*100,0.1,10000,1.1,1.0])
    theta_tor = torch.tensor(theta,requires_grad=False)
    cost_mat_tor = torch.tensor(total_constrained_sim.cost_matrix,requires_grad=False)
    ...
    _,true_grad_tor,_,_ = torch.autograd.functional.jacobian(test4_helpers.pytorch_total_constrained_sde_potential, 
...
def test_data_likelihood_and_gradient(test4_helpers,total_constrained_sim):
    # Create theta
    np.random.seed(1234)
    theta = np.array([1.0,0.5*100,0.1,10000,1.1,1.0])
    yy = total_constrained_sim.log_destination_attraction
    xx = yy + np.log(np.random.uniform(low=0.1, high=1.0, size=1))
    theta_tor = torch.tensor(theta,requires_grad=False)
    cost_mat_tor = torch.tensor(total_constrained_sim.cost_matrix,requires_grad=False)
    yy_tor = torch.tensor(yy,requires_grad=False)
    xx_tor = torch.tensor(xx,requires_grad=True)
    noise_var_tor = torch.tensor(total_constrained_sim.noise_var)
    
    # Compute log(p(y|x,theta))
    negative_log_likelihood, negative_gradLogLikelihood = total_constrained_sim.negative_destination_attraction_log_likelihood_and_gradient(xx,1./total_constrained_sim.noise_var)
